          ...
          <div className="mb-8">
            <div className="flex items-center space-x-4 mb-4">
              <div 
                ...
                style={{ backgroundColor: displayMailingList.color }}
              />
            ...
            <div className="flex items-center space-x-6 text-sm text-gray-500">
              <div className="flex items-center space-x-2">
                ...
                <span>{displayMailingList.subscriber_count?.toLocaleString() || 'N/A'} subscribers</span>
              </div>
              ...
              {displayMailingList.last_sent_at && (
                <div className="flex items-center space-x-2">
                  ...
                  <span>Last sent {new Date(displayMailingList.last_sent_at).toLocaleDateString()}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <main>
          {emails.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500">This mailing list doesn't have any emails yet.</p>
            </div>
          ) : (
            <div className="space-y-12">
              {emails.map((email, index) => (
                <EmailCard key={email.id} email={email} showMailingList={false} index={index} />
              ))}
            </div>
          )}
        </main>
